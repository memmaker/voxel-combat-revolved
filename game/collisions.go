...
type CollidingObject interface {
	...
	GetColliders() []util.Collider
	GetName() string
	SetVelocity(newVelocity mgl32.Vec3)
	HitWithProjectile(projectile CollidingObject, part util.Collider)
	IsDead() bool
}
type CollisionSolver struct {
	...
	objectsOnGrid           map[util.IntVec3]map[CollidingObject]bool
...
func NewCollisionSolver(isSolid func(x int32, y int32, z int32) bool) *CollisionSolver {
	return &CollisionSolver{
		...
		objectsOnGrid:           make(map[util.IntVec3]map[CollidingObject]bool),
...
func (c *CollisionSolver) Update(deltaTime float64) {
	...
	for _, obj := range c.allObjects {
		...
		correctedPos, isCollidingWithMap := util.AABBVoxelMapIntersection(previousPos, newPos, obj.GetExtents(), c.isSolid)
...
func (c *CollisionSolver) updateProjectiles(deltaTime float64) {
	...
	for pIndex := len(c.allProjectiles) - 1; pIndex >= 0; pIndex-- {
		...
		rayHitObject := false
		var nearestPart util.Collider
		var nearestPoint mgl32.Vec3
		var nearestObject CollidingObject
		firstGridCell := true
		var lastGridCell util.IntVec3
		var gridCell util.IntVec3
		rayHitInfo := util.DDARaycast(previousPos, newPos, func(x, y, z int) bool {
			if c.isSolid(int32(x), int32(y), int32(z)) {
				return true
			}
			gridCell = util.IntVec3{x, y, z}.Div(2)
...
func (c *CollisionSolver) DoAABBNarrowPhase(deltaTime float64, obj CollidingObject, other map[CollidingObject]bool, buffer []CollidingObject) (bool, mgl32.Vec3, []CollidingObject) {
	...
	for otherObj, _ := range other {
		...
		mMin, mExtents := util.AABBMinkowskiDifference(prevMin, obj.GetExtents(), otherPrevMin, otherObj.GetExtents())
		minkowskiAABB := util.NewAABBFromMin(mMin, mExtents)

		// 5. Do narrow phase by checking all AABBs against each other
		colInfo := util.SweepAABBFromMinkowski(minkowskiAABB, combinedRelativeVelocity)
...
func (c *CollisionSolver) addToGrid(atPos mgl32.Vec3, obj CollidingObject) {
	...
	ttl := util.IntVec3{int(atPos.X() - extents.X()), int(atPos.Y() + extents.Y()), int(atPos.Z() - extents.Z())}.Div(2)
	ttr := util.IntVec3{int(atPos.X() + extents.X()), int(atPos.Y() + extents.Y()), int(atPos.Z() - extents.Z())}.Div(2)
	tbl := util.IntVec3{int(atPos.X() - extents.X()), int(atPos.Y() + extents.Y()), int(atPos.Z() + extents.Z())}.Div(2)
	tbr := util.IntVec3{int(atPos.X() + extents.X()), int(atPos.Y() + extents.Y()), int(atPos.Z() + extents.Z())}.Div(2)

	// bottom face corners (btl, btr, bbl, bbr)
	btl := util.IntVec3{int(atPos.X() - extents.X()), int(atPos.Y() - extents.Y()), int(atPos.Z() - extents.Z())}.Div(2)
	btr := util.IntVec3{int(atPos.X() + extents.X()), int(atPos.Y() - extents.Y()), int(atPos.Z() - extents.Z())}.Div(2)
	bbl := util.IntVec3{int(atPos.X() - extents.X()), int(atPos.Y() - extents.Y()), int(atPos.Z() + extents.Z())}.Div(2)
	bbr := util.IntVec3{int(atPos.X() + extents.X()), int(atPos.Y() - extents.Y()), int(atPos.Z() + extents.Z())}.Div(2)
...
func (c *CollisionSolver) getGridObjectsNear(pos mgl32.Vec3, collidingObject CollidingObject) map[CollidingObject]bool {
	...
	// top face corners (ttl, ttr, tbl, tbr)
	ttl := util.IntVec3{int(pos.X() - extents.X()), int(pos.Y() + extents.Y()), int(pos.Z() - extents.Z())}.Div(2)
	ttr := util.IntVec3{int(pos.X() + extents.X()), int(pos.Y() + extents.Y()), int(pos.Z() - extents.Z())}.Div(2)
	tbl := util.IntVec3{int(pos.X() - extents.X()), int(pos.Y() + extents.Y()), int(pos.Z() + extents.Z())}.Div(2)
	tbr := util.IntVec3{int(pos.X() + extents.X()), int(pos.Y() + extents.Y()), int(pos.Z() + extents.Z())}.Div(2)

	// bottom face corners (btl, btr, bbl, bbr)
	btl := util.IntVec3{int(pos.X() - extents.X()), int(pos.Y() - extents.Y()), int(pos.Z() - extents.Z())}.Div(2)
	btr := util.IntVec3{int(pos.X() + extents.X()), int(pos.Y() - extents.Y()), int(pos.Z() - extents.Z())}.Div(2)
	bbl := util.IntVec3{int(pos.X() - extents.X()), int(pos.Y() - extents.Y()), int(pos.Z() + extents.Z())}.Div(2)
	bbr := util.IntVec3{int(pos.X() + extents.X()), int(pos.Y() - extents.Y()), int(pos.Z() + extents.Z())}.Div(2)
...
func (c *CollisionSolver) onProjectileObjectCollision(projectile CollidingObject, collidingObject CollidingObject, part util.Collider, worldPosOfCollision mgl32.Vec3) {
...
func (c *CollisionSolver) onProjectileMapCollision(projectile CollidingObject, info util.HitInfo3D) {
	projectile.SetVelocity(mgl32.Vec3{0, 0, 0})
